...
const Charts: React.FC = (props) => {
  ...
  useEffect(() => {
    ...
    const fetchResult = async () => {
      const url =
        "/getCovid19InfStateJson/Wo5oDrq3DoodeD%2FxyAuH9kIcx5z7iQcIssa6YvEkuW6H1Ssn0e2IWdcgsIZtaynpv1pDV%2F%2FoLUQ3A45r9Yhzsw%3D%3D&pageNo=1&numOfRows=10&startCreateDt=20210809&endCreateDt=20210811";
      ...
      setData((data) => {
        return {
          ...data,
          labels: [
            ...json.response.body.items.item.map((i) => +i.stateDt._text),
          ],
          datasets: [
            {
              label: "확진자 수",
              data: [
                ...json.response.body.items.item.map((i) => +i.decideCnt._text),
              ],
              backgroundColor: [
                "rgba(255, 99, 132, 0.2)",
                "rgba(255, 99, 132, 0.2)",
